...
DUMP_FILE = 'data/simulator_ddpg_dump.txt'
...
class rhavok_simulator(Thread):
    ...
    def run(self):
        ...
        hwm = mowin(self.U_FILE, self.S_FILE, mwin = init_mw, thres = 0.002, idx = [0,7])  # brunton idx = [0,14]
        ...
        if not self.semaphore: # wait to start simulation
            ...
            while True:
                if self.semaphore: break
        
        if DUMP_FILE is not None:  # dump coordinate to file if requested
            file_object = open(DUMP_FILE, 'a')
...
print('[ INFO ] model simulation online!')
...
def stream():  # read data from buffer & update plots
    ...
    action_monitor_txt = '<b style="color:red;">forcing</b>' if action_active else '<b style="color:blue;">waiting</b>'
    info_div.text = info_div_base.format(havok_monitor_txt, action_monitor_txt)
...
bt = Button(label='start/pause', aspect_ratio=2)
...
head_text = [
    ...
    #Div(text=r"Laboratory of Computational Physics, module B\n<b>Group 2202</b> // Barone, Nagaro, Ninni, Valentini"),
    Div(text="""coder: Francesco Pio Barone, git@baronefr""")
]

# system controller info
info_div_base = "<b>System controllers</b><br> HAVOK sentinel: {}<br> action: {}"
info_div = Div( text=info_div_base.format('-','-'), width=500, height=200, style = {"font-size": "150%"})

# CREATE PAGE
doc = curdoc()
doc.add_periodic_callback(stream, 100)

doc.add_root( column( column(head_text), 
                      row(column(p_coords,bt,column(info_div)), column(p_havok_vr, p_action, p_reward)), 
                    ) 
            )
doc.title = "reinforced HAVOK sentinel"
